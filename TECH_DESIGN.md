...
**사용자가 하는 건 딱 두 가지: 값 바꾸기, 코드 붙여넣기. 나머지는 전부 자동.**

### 자동화 흐름 1: 토큰 → 배포
```
사용자: 브랜드 색상 #3182F6 → #FF6F00 변경 (컬러피커 클릭)
  ↓ 자동
① theme.js / colors.js 파일 내용 갱신
  ↓ 자동
② GitHub 커밋 "토큰 업데이트: brand #3182F6 → #FF6F00"
  ↓ 자동
③ Vercel 빌드 트리거
  ↓ 자동
④ 배포 완료 → 프리뷰 URL 갱신
...
사용자: App.jsx 코드 붙여넣기
  ↓ 자동
① import 구문 파싱 ("./theme.js", "./data.js" 등 추출)
  ↓ 자동
② 프로젝트 파일에서 해당 파일 검색
  ↓ 자동 (파일 없으면)
③ "theme.js가 필요합니다" → 토큰 세트에서 자동 생성
  ↓ 자동
④ 모든 파일을 Sandpack에 전달 → 번들링
  ↓ 자동
⑤ 미리보기에 즉시 렌더링

사용자가 인식하는 것: "코드 붙여넣었더니 화면이 보인다" — 끝.
```

### 자동화 흐름 3: 컴포넌트 수정 → 전파
```
사용자: 컴포넌트 탭에서 "Primary 버튼" 색상 변경
  ↓ 자동
① 해당 컴포넌트 코드 업데이트
  ↓ 자동
② 이 컴포넌트를 사용하는 모든 시안에 자동 반영
  ↓ 자동
③ GitHub 커밋 + Vercel 재배포
  ↓ 자동
④ 미리보기 갱신

사용자가 인식하는 것: "버튼 하나 바꿨더니 전체가 바뀌었다" — 끝.
```

### 구현 필수 조건
- **디바운스:** 변경 후 1~2초 대기 후 자동 실행 (매 키스트로크마다 X)
- **큐잉:** 여러 변경이 연속되면 마지막 것만 커밋/배포
- **상태 표시:** 하단 바에 "저장 중..." → "커밋 중..." → "배포 중..." → "✓ 완료"
- **실패 처리:** GitHub/Vercel 에러 시 토스트 알림 + 재시도 버튼
- **오프라인 대응:** 네트워크 없으면 localStorage에 저장, 복구 시 자동 동기화
...
> **자동화 필수 원칙:** 모든 세션에서 "사용자 조작 → 나머지 전부 자동" 원칙 적용.
> 사용자가 하는 건 값 바꾸기와 코드 붙여넣기뿐. 저장/커밋/배포/렌더링은 자동.
> TECH_DESIGN.md의 "0. 자동화 원칙" 섹션 반드시 먼저 읽을 것.
...
특히 TECH_DESIGN.md의 "0. 자동화 원칙"을 반드시 숙지.

자동화 원칙: 사용자가 하는 건 값 바꾸기와 코드 붙여넣기뿐.
저장/커밋/배포/렌더링은 전부 자동이어야 함.
...
3. storage.js를 Supabase로 교체 (localStorage → DB)
   - 변경사항 자동 저장 (디바운스 1초)
   - 오프라인 시 localStorage fallback → 복구 시 자동 동기화
4. 기존 데이터 마이그레이션 (INIT_PROJECTS → Supabase)
5. 로그인 UI 추가 (Supabase Auth, GitHub OAuth)
6. 하단 상태바 추가: "저장 중..." → "✓ 저장됨" 표시

Supabase 프로젝트 URL: [여기에 입력]
Supabase Anon Key: [여기에 입력]
```

### 세션 2: GitHub 연동 + 자동 커밋
```
너는 전문 프론트엔드/백엔드 엔지니어야.
Artifact Studio 프로젝트를 이어서 작업합니다.
TECH_DESIGN.md의 "0. 자동화 원칙" 반드시 숙지.

자동화 원칙: 파일 수정 → GitHub 자동 커밋. 사용자가 git 명령어를 치는 일 없음.

이번 세션 목표:
1. @octokit/rest 설치
2. GitHub OAuth 인증 플로우 (PAT 저장 → Supabase profiles 테이블)
3. 파일 트리 읽기 API
4. 파일 읽기/쓰기/커밋 기능
5. 에디터 좌측에 GitHub 파일 탐색기 사이드바 추가
6. **자동 커밋 파이프라인 구현:**
   - 토큰 변경 → colors.js/theme.js 자동 생성 → GitHub 자동 커밋
   - 시안 코드 변경 → 해당 파일 자동 커밋
   - 디바운스: 변경 후 3초 대기 후 실행
   - 큐잉: 여러 변경이 연속되면 마지막 것만 커밋
   - 커밋 메시지 자동 생성 (예: "토큰 업데이트: brand #3182F6 → #FF6F00")
7. 하단 상태바: "저장 중..." → "커밋 중..." → "✓ 커밋됨 (abc1234)"

GitHub 저장소: dudmlwnqh/artifact-studio
```

### 세션 3: 디자인 토큰 시스템 + 자동 생성
```
너는 전문 프론트엔드/백엔드 엔지니어야.
Artifact Studio 프로젝트를 이어서 작업합니다.
TECH_DESIGN.md의 "0. 자동화 원칙" 반드시 숙지.
...
자동화 원칙:
- 색상 바꾸면 → theme.js 자동 갱신 → GitHub 자동 커밋 → 미리보기 즉시 반영
- 사용자가 "저장" 버튼을 누르는 일 없음. 전부 자동.
...
5. **토큰 → 코드 파일 자동 생성 파이프라인:**
   - 토큰 세트 변경 감지 (디바운스 1초)
   - colors.js, typography.js, spacing.js, theme.js 자동 생성
   - GitHub 자동 커밋 (세션2에서 만든 파이프라인 활용)
   - Sandpack에 자동 전달 → 미리보기 즉시 반영
6. color-thief 설치 & 이미지 색상 추출
7. 랜덤 셔플 기능
8. 컴포넌트 수정 → 사용하는 모든 시안에 자동 반영

참고: TECH_DESIGN.md의 "디자인 소스 탭 설계" 섹션
```

### 세션 4: JSX 풀 렌더링 + Vercel 자동 배포
```
너는 전문 프론트엔드/백엔드 엔지니어야.
Artifact Studio 프로젝트를 이어서 작업합니다.
TECH_DESIGN.md의 "0. 자동화 원칙" 반드시 숙지.
...
자동화 원칙:
- 코드 붙여넣기 → import 자동 해석 → 누락 파일 자동 생성 → 렌더링
- GitHub 커밋 → Vercel 자동 배포 → 프리뷰 URL 자동 갱신
- 사용자가 터미널/CLI를 만지는 일 없음. 전부 웹 안에서 자동.

이번 세션 목표:
1. **import 자동 해석 파이프라인:**
   - 코드에서 import 경로 추출 (정규식 파서)
   - 프로젝트 파일에서 자동 검색
   - 없으면: 토큰 세트 → theme.js 자동 생성, 컴포넌트 탭 → 해당 컴포넌트 파일 생성
   - 여전히 없으면: "이 파일이 필요합니다" 안내 (+ 코드 입력 UI)
2. 프로젝트 파일 + GitHub 파일 → Sandpack files 자동 통합
3. App.jsx 넣으면 모든 import 자동 해결 → 렌더링
4. **Vercel 자동 배포 파이프라인:**
   - Vercel API 토큰 입력 (최초 1회)
   - GitHub 커밋 감지 → 자동 빌드 트리거
   - 빌드 상태 실시간 표시 (하단 바)
   - 배포 완료 → 프리뷰 URL 자동 갱신
5. 하단 상태바 통합: "저장됨 → 커밋됨 → 빌드 중... → ✓ 배포됨 (url)"

실패 처리:
- 네트워크 에러 → 토스트 알림 + 재시도 버튼
- 빌드 실패 → 에러 로그 표시 + "이전 버전 유지" 안내
- 오프라인 → localStorage 큐잉 → 복구 시 자동 동기화
```
